...
public class FrmBusquedaLibro extends javax.swing.JFrame {
    ...
    // Lógica para buscar libros en la lista de libros cargados desde el archivo
  List<Libro> resultados = new ArrayList<>();

    try (var mongoClient = MongoClients.create("mongodb://localhost:27017")) {
        MongoDatabase database = mongoClient.getDatabase("Libreria");
        MongoCollection<Document> collection = database.getCollection("economia");
        ...
        try (MongoCursor<Document> cursor = collection.find(query).iterator()) {
            while (cursor.hasNext()) {
                ...
                libro.setDisponible(true); // Debes obtener el estado de disponibilidad desde la base de datos
                libro.setStock(0); // Debes obtener el stock desde la base de datos
    ...
    private void mostrarResultadosEnTabla(List<Libro> resultados) {
   ...
   try (var mongoClient = MongoClients.create("mongodb://localhost:27017")) {
        ...
        Document query = new Document("genero", libroId);
        try (MongoCursor<Document> cursor = collection.find(query).iterator()) {
            if (cursor.hasNext()) {
                Document libroDoc = cursor.next();
    ...
    private void guardarLibrosEnArchivo(List<Libro> libros) {
    try (PrintWriter writer = new PrintWriter(new FileWriter("libro_data.csv"))) {
        for (Libro libro : libros) {
            String csvData = libro.toCsv();
